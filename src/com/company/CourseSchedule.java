...
public class CourseSchedule {
    ...
    public static boolean canFinish(int numCourses, int[][] prerequisites) {
        ...
        for (int i = 0; i < numCourses; i++) {
            if (!dfs(list, i, new boolean[numCourses]))
                return false;
        }
        return true;
    }

    private static boolean dfs(ArrayList<Integer>[] list, int course, boolean[] visited) {
        if (visited[course])
            return false;
        else
            visited[course] = true;
    ...
    public static int[] findOrder(int numCourses, int[][] prerequisites) {
        ...
        for (int i = 0; i < numCourses; i++) {
            if (!dfs2(list, i, new boolean[numCourses], resultList))
                return new int[numCourses];
        }
        return resultList.stream().mapToInt(i -> i).toArray();
    }

    private static boolean dfs2(ArrayList<Integer>[] list, int course, boolean[] visited, ArrayList<Integer> resultList) {
        if (visited[course])
            return false;
        else {
            if(resultList.size() < list.length)
                resultList.add(course);
            visited[course] = true;
        }

        for (int c : list[course]) {
            if (!dfs2(list, c, visited, resultList))
                return false;
        }
        visited[course] = false;
        return true;
